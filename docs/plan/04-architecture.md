...
> As-built. This describes the system that shipped: a **multi-page static site** running on the **CourseKit** engine. (It replaces an earlier single-page hash-router design; the move to real per-page URLs is covered in `05-i18n-seo.md`, and the engine extraction in `06-coursekit-framework.md`.) `CLAUDE.md` is the canonical quick reference; this doc is the deeper "why it's shaped this way".

## 1. Big picture

**Multi-page static** (for SEO, no build step) on top of **CourseKit**, a theme-agnostic engine that everything attaches to via the single global `window.CourseKit`. A *course* supplies only **data + config + theme**; the engine renders the home page, lessons, quizzes, scoring, progress, and i18n. The Google Wire course is the consumer **at the repo root** (so its public URLs stay at the origin root); the engine lives in `core/` + `themes/editorial/`.

```
/                              repo root = the Wire course (the consumer)
  ...
  core/                        CourseKit engine (theme-agnostic)
  ...
  examples/starter/            second course proving engine reuse (no engine edits)
```
...
- **CourseKit engine** under one namespace (`window.CourseKit`); the course supplies only data + config + theme.
